...
class Tainacan_Blocksy_Textarea_Readmore {
	...
	public $meta_key = 'tainacan_blocksy_textarea_readmore_max_chars';
	...
	public function form() {
		...
		ob_start();
		?>
		<div class="field tainacan-blocksy-textarea-readmore--section-header">
			<h4><?php esc_html_e( 'Integration of Tainacan into the Blocksy theme', 'tainacan-blocksy' ); ?></h4>
			<hr>
		</div>
		<div class="field tainacan-blocksy-textarea-readmore--max-chars">
			<label class="label" for="tainacan-blocksy-textarea-readmore-max-chars">
				<?php esc_html_e( 'Maximum characters to show before “Read more”', 'tainacan-blocksy' ); ?>
			</label>
			<div class="control">
				<input
					type="number"
					...
					min="1"
					class="input"
					id="tainacan-blocksy-textarea-readmore-max-chars"
					name="<?php echo esc_attr( $this->meta_key ); ?>"
					placeholder="<?php esc_attr_e( 'No limit', 'tainacan-blocksy' ); ?>"
				/>
			</div>
			<p class="help">
				<?php esc_html_e( 'Leave empty to always show the full text on the item page. If set, long values show a short preview with a control to expand.', 'tainacan-blocksy' ); ?>
			</p>
		</div>
		<?php
		return ob_get_clean();
	...
	private function wrap_readmore_widget( $preview_html, $full_html, $metadatum_id, $index ) {
		...
		$region_id = 'tainacan-blocksy-tm-readmore-full-' . (int) $metadatum_id . '-' . (int) $index;

		ob_start();
		?>
		<div class="tainacan-blocksy-textarea-readmore">
			<div class="tainacan-blocksy-textarea-readmore__preview">
			...
			<div
				class="tainacan-blocksy-textarea-readmore__full"
			...
			<a
				...
				role="button"
				class="tainacan-blocksy-textarea-readmore__toggle tainacan-blocksy-textarea-readmore__toggle--more"
				aria-expanded="false"
	...
	private function maybe_enqueue_readmore_assets() {
		...
		wp_enqueue_script(
			...
			TAINACAN_BLOCKSY_PLUGIN_URL_PATH . '/js/textarea-readmore.js',
			array(),
...
/**
 * Registers after theme setup so another read-more implementation can load first.
 */
function tainacan_blocksy_init_textarea_readmore() {
	if ( ! defined( 'TAINACAN_VERSION' ) ) {
		return;
	}
	if ( class_exists( 'Tainacan_Interface_Textarea_Readmore', false ) ) {
		return;
	}
	new Tainacan_Blocksy_Textarea_Readmore();
}
add_action( 'after_setup_theme', 'tainacan_blocksy_init_textarea_readmore', 20 );
