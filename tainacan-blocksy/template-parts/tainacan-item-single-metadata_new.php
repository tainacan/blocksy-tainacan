<?php

// phpcs:disable WordPress.WP.I18n.TextDomainMismatch -- All translatable strings in this file reuse translations from the Tainacan plugin.
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

// phpcs:disable WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Invokes Tainacan core hooks.

    $tainacan_blocksy_prefix = blocksy_manager()->screen->get_prefix();
    
    $tainacan_blocksy_section_layout = get_theme_mod($tainacan_blocksy_prefix . '_metadata_sections_layout_type', 'metadata-section-type-1');
    $tainacan_blocksy_exclude_title_metadata = get_theme_mod($tainacan_blocksy_prefix . '_show_title_metadata', 'yes') === 'no';
    $tainacan_blocksy_show_thumbnail_with_metadata = get_theme_mod($tainacan_blocksy_prefix . '_show_thumbnail', 'no') === 'yes';
    $tainacan_blocksy_metadata_list_structure_type = get_theme_mod($tainacan_blocksy_prefix . '_metadata_list_structure_type', 'metadata-type-1');
    $tainacan_blocksy_display_section_labels = get_theme_mod($tainacan_blocksy_prefix . '_display_section_labels', 'yes') == 'yes';
    $tainacan_blocksy_show_default_section_separated =
        in_array($tainacan_blocksy_section_layout, ['metadata-section-type-2', 'metadata-section-type-3', 'metadata-section-type-4']) &&
        get_theme_mod($tainacan_blocksy_prefix . '_metadata_sections_separate_default_section', 'no') === 'yes';


    /** 
     * The new metadata sections function makes it a bit more complicated to add
     * the thumbnail in the middle of the metadata.
     * So we have some logic that is only needed if it is set.
     * The following uses a filter to add it right above the first metadatum in the default section.
     **/
    if ( has_post_thumbnail() && $tainacan_blocksy_show_thumbnail_with_metadata ) {

        add_filter('tainacan-get-metadata-section-as-html-before-metadata-list--index-0', function( $tainacan_blocksy_before_description, $tainacan_blocksy_metadata_section) {
            
            ob_start();
            ?>
                <div class="tainacan-item-section__metadata-thumbnail">
                    <h3 class="tainacan-metadata-label"><?php esc_html_e( 'Thumbnail', 'tainacan' ); ?></h3>
                    <p class="tainacan-metadata-value"><?php the_post_thumbnail('tainacan-medium-full'); ?></p>
                </div>
            <?php
    
            $tainacan_blocksy_extra_content = ob_get_contents();
            ob_end_clean();
    
            return $tainacan_blocksy_before_description . $tainacan_blocksy_extra_content;

        }, 10, 2);

    }

    $tainacan_blocksy_metadata_args = array(
        'display_slug_as_class' => true,
        'before' 				=> '<div class="tainacan-item-section__metadatum metadata-type-$type" $id>',
		'after' 				=> '</div>',
        'before_title' => '<h3 class="tainacan-metadata-label">',
        'after_title' => '</h3>',
        'before_value' => '<p class="tainacan-metadata-value">',
        'after_value' => '</p>',
        'exclude_title' => $tainacan_blocksy_exclude_title_metadata
    );

    echo '<div class="tainacan-item-section tainacan-item-section--metadata-sections">';

        if ( $tainacan_blocksy_show_default_section_separated ) {
            $tainacan_blocksy_sections_args = array(
                'metadata_sections__in' => [ \Tainacan\Entities\Metadata_Section::$tainacan_blocksy_default_section_slug ],
                'before' => '<section class="tainacan-item-section tainacan-item-section--metadata">',
                'after' => '</section>',
                'before_name' => '<h2 class="tainacan-single-item-section" id="metadata-section-$slug">',
                'after_name' => '</h2>',
                'hide_name' => !$tainacan_blocksy_display_section_labels,
                'before_metadata_list' => do_action( 'tainacan-blocksy-single-item-metadata-begin' ) . '<div class="tainacan-item-section__metadata ' . $tainacan_blocksy_metadata_list_structure_type . '">',
                'after_metadata_list' => '</div>' . do_action( 'tainacan-blocksy-single-item-metadata-end' ),
                'metadata_list_args' => $tainacan_blocksy_metadata_args
            );
            
            tainacan_the_metadata_sections( $tainacan_blocksy_sections_args );
        }
        
        if ( $tainacan_blocksy_section_layout == 'metadata-section-type-2') {

            add_filter('tainacan-get-metadata-section-as-html-before-name--index-0', function($tainacan_blocksy_before, $tainacan_blocksy_item_metadatum) {
                return str_replace('<input', '<input checked="checked"', $tainacan_blocksy_before);
            }, 10, 2);

            $tainacan_blocksy_sections_args = array(
                'metadata_sections__not_in' => $tainacan_blocksy_show_default_section_separated ? [ \Tainacan\Entities\Metadata_Section::$tainacan_blocksy_default_section_slug ] : [],
                'before' => '',
                'after' => '',
                'before_name' => '<input name="tabs" type="radio" id="tab-section-$id" />
                            <label for="tab-section-$id">
                                <h2 class="tainacan-single-item-section" id="metadata-section-$slug">',
                'after_name' => '</h2>
                            </label>',
                'before_metadata_list' => '<section class="tainacan-item-section tainacan-item-section--metadata">' . do_action( 'tainacan-blocksy-single-item-metadata-begin' ) . '
                        <div class="tainacan-item-section__metadata ' . $tainacan_blocksy_metadata_list_structure_type . '" aria-labelledby="metadata-section-$slug">',
                'after_metadata_list' => '</div>' . do_action( 'tainacan-blocksy-single-item-metadata-end' ) . '</section>',
                'metadata_list_args' => $tainacan_blocksy_metadata_args
            );
            
            echo '<div class="metadata-section-layout--tabs">';
            tainacan_the_metadata_sections( $tainacan_blocksy_sections_args );
            echo '</div>';

        } else  if ( $tainacan_blocksy_section_layout == 'metadata-section-type-3') {

            wp_enqueue_style( 'tainacan-icons' );

            add_filter('tainacan-get-metadata-section-as-html-before-name--index-0', function($tainacan_blocksy_before, $tainacan_blocksy_item_metadatum) {
                return str_replace('<input', '<input checked="checked"', $tainacan_blocksy_before);
            }, 10, 2);

            $tainacan_blocksy_sections_args = array(
                'metadata_sections__not_in' => $tainacan_blocksy_show_default_section_separated ? [ \Tainacan\Entities\Metadata_Section::$tainacan_blocksy_default_section_slug ] : [],
                'before' => '',
                'after' => '',
                'before_name' => '<input name="collapses" type="checkbox" id="collapse-section-$id"/>
                            <label for="collapse-section-$id">
                                <i class="tainacan-icon tainacan-icon-arrowright" aria-hidden="true"></i>
                                <h2 class="tainacan-single-item-section" id="metadata-section-$slug">',
                'after_name' => '</h2>
                            </label>',
                'before_metadata_list' => '<section class="tainacan-item-section tainacan-item-section--metadata">' . do_action( 'tainacan-blocksy-single-item-metadata-begin' ) . '
                    <div class="tainacan-item-section__metadata ' . $tainacan_blocksy_metadata_list_structure_type . '" aria-labelledby="metadata-section-$slug">',
                'after_metadata_list' => '</div>' . do_action( 'tainacan-blocksy-single-item-metadata-end' ) . '</section>',
                'metadata_list_args' => $tainacan_blocksy_metadata_args
            );

            echo '<div class="metadata-section-layout--collapses">';
            tainacan_the_metadata_sections( $tainacan_blocksy_sections_args );
            echo '</div>';

        } else if ( $tainacan_blocksy_section_layout == 'metadata-section-type-4') {

            wp_enqueue_style( 'tainacan-icons' );

            add_filter('tainacan-get-metadata-section-as-html-before-name--index-0', function($tainacan_blocksy_before, $tainacan_blocksy_item_metadatum) {
                return str_replace('<input', '<input checked="checked"', $tainacan_blocksy_before);
            }, 10, 2);

            $tainacan_blocksy_sections_args = array(
                'metadata_sections__not_in' => $tainacan_blocksy_show_default_section_separated ? [ \Tainacan\Entities\Metadata_Section::$tainacan_blocksy_default_section_slug ] : [],
                'before' => '',
                'after' => '',
                'before_name' => '<input name="accordion" type="radio" id="accordion-section-$id"/>
                            <label for="accordion-section-$id">
                                <i class="tainacan-icon tainacan-icon-arrowright" aria-hidden="true"></i>
                                <h2 class="tainacan-single-item-section" id="metadata-section-$slug">',
                'after_name' => '</h2>
                            </label>',
                'before_metadata_list' => '<section class="tainacan-item-section tainacan-item-section--metadata">' . do_action( 'tainacan-blocksy-single-item-metadata-begin' ) . '
                    <div class="tainacan-item-section__metadata ' . $tainacan_blocksy_metadata_list_structure_type . '" aria-labelledby="metadata-section-$slug">',
                'after_metadata_list' => '</div>' . do_action( 'tainacan-blocksy-single-item-metadata-end' ) . '</section>',
                'metadata_list_args' => $tainacan_blocksy_metadata_args
            );

            echo '<div class="metadata-section-layout--accordion">';
            tainacan_the_metadata_sections( $tainacan_blocksy_sections_args );
            echo '</div>';

        } else {
            $tainacan_blocksy_sections_args = array(
                'metadata_sections__not_in' => $tainacan_blocksy_show_default_section_separated ? [ \Tainacan\Entities\Metadata_Section::$tainacan_blocksy_default_section_slug ] : [],
                'before' => '<section class="tainacan-item-section tainacan-item-section--metadata">',
                'after' => '</section>',
                'before_name' => '<h2 class="tainacan-single-item-section" id="metadata-section-$slug">',
                'after_name' => '</h2>',
                'hide_name' => !$tainacan_blocksy_display_section_labels,
                'before_metadata_list' => do_action( 'tainacan-blocksy-single-item-metadata-begin' ) . '<div class="tainacan-item-section__metadata ' . $tainacan_blocksy_metadata_list_structure_type . '">',
                'after_metadata_list' => '</div>' . do_action( 'tainacan-blocksy-single-item-metadata-end' ),
                'metadata_list_args' => $tainacan_blocksy_metadata_args
            );
            
            tainacan_the_metadata_sections( $tainacan_blocksy_sections_args );
        }

    echo '</div>';
?>
